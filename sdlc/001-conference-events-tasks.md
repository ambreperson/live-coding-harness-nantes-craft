...
> **Point d'attention signalé par l'agent `documenter`, corrigé depuis** : la règle ArchUnit `domains_must_not_reach_into_other_domains_internals` de `HexagonalArchitectureTest` ne détectait en réalité aucune violation — bug de polarité dans le `ArchCondition` personnalisé combiné à `noClasses()` (qui inverse la polarité de chaque événement émis ; la condition émettait `violated(...)` pour le cas fautif, ce qui, une fois inversé, ne faisait jamais échouer la règle). Corrigé en émettant `satisfied(...)` à la place, puis vérifié expérimentalement (violation injectée puis retirée). La correction a fait apparaître une violation réelle déjà présente : `proposal` dépendait directement du `domain/model` d'`event` (`EventNotFoundException`, `Event` dans les tests) au lieu de passer uniquement par son `port/in`. Plutôt que d'exempter la règle, le code a été refactoré : nouveau `event.domain.port.in.CheckEventExistsUseCase` (`exists(eventId): boolean`) + `CheckEventExistsService`, et `proposal` porte désormais sa propre `EventNotFoundException` (classe distincte de celle d'`event`). `SubmitProposalService` ne dépend plus que de `port/in` d'`event`. Suite complète (61 tests) verte, `ARCHITECTURE.md`/`CLAUDE.md` mis à jour en conséquence.
