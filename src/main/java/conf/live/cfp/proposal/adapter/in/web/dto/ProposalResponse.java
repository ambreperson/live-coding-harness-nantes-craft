 ...
 */
public record ProposalResponse(String id, String title, String description, String speakerId, ProposalStatus status) {

	public static ProposalResponse from(Proposal proposal) {
		return new ProposalResponse(proposal.id(), proposal.title(), proposal.description(), proposal.speakerId(), proposal.status());
	}
}
